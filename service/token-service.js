...
class TokenService {
    ...
    async saveToken(user_id, refresh_token) {
        const token_data = await UserTokens.findOne({user_id})
        if (token_data) {
            token_data.refresh_token = refresh_token;
            return token_data.save()
        }
        return await UserTokens.create({user_id, refresh_token});
    }
    ...
    async findToken(refresh_token) {
        return await UserTokens.findOne({where: {refresh_token}});
    }
}
